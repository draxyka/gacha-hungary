  ...
  return (
    ...
    >
      <div className="wrapper relative z-1">
        ...
        {/* Felső sor: Hamarosan (bal) + 3 kártya (jobb) */}
        <div className="flex flex-col lg:flex-row gap-6">
          {/* Hírek kártya */}
          <Link
            href={`/${slug}/hirek`}
            className={`card text-gray-200 rounded-2xl relative flex items-center gap-4 px-8 py-8 z-2 lg:flex-[1.4] bg-white/5 backdrop-blur-sm border ${cardBorder} no-underline transition-all duration-300 ease-in-out hover:shadow-2xl`}
          >
            <div className="relative z-1 flex-1 min-w-0" style={{ textShadow: '0 2px 12px rgba(0,0,0,1), 0 0 20px rgba(0,0,0,0.9), 0 0 4px rgba(0,0,0,1)' }}>
              <h2 className={`w-full font-semibold tracking-[0.2em] uppercase text-3xl lg:text-4xl ${accentColor}`}>
                HÍREK
              </h2>
              <p className="mt-4 text-gray-400 text-md leading-relaxed max-w-[280px]">
                Legfrissebb hírek és bejelentések magyarul.
              </p>
            </div>
            <figure className="card-image w-[35%] sm:w-[40%] shrink-0 absolute right-4 bottom-0 h-full z-0 flex items-end justify-end transition-all duration-300 ease-in-out opacity-50 sm:opacity-80">
              <Image
